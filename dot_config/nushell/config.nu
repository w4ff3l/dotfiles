...
use ~/.config/nushell/gruvbox-dark.nu *
...
$env.config = {
  ...
  color_config: (gruvbox_dark)   # if you want a light theme, replace `$dark_theme` to `$light_theme`
  use_grid_icons: true
...
alias glg = git log --graph -n 10
alias glgo = git log --graph --oneline -n 10

ssh-agent -c | lines | first 2 | parse "setenv {name} {value};" | transpose -i -r -d | load-env
# ssh-add -q ~/.ssh/<your-key>
